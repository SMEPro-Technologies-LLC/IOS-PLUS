...
def emit_sql(nodes, path: Path):
    ...
    lines = [
        ...
        f"-- Generated: {date.today().isoformat()} | nodes in this seed: {len(nodes)}",
...
def load_freeze_list(path, report):
    """Accepts either the review-queue API export (list of rows with
    uco_node_id [+ amendment_id]) or a bare list of node-id strings.
    Returns {node_id: reason}."""
    raw = json.loads(Path(path).read_text(encoding="utf-8"))
    frozen = {}
    if not isinstance(raw, list):
        sys.exit(f"--freeze-list: expected a JSON array, got {type(raw).__name__}")
    for item in raw:
        if isinstance(item, str):
            frozen[item] = "frozen via freeze list"
        elif isinstance(item, dict) and item.get("uco_node_id"):
            aid = item.get("amendment_id", "?")
            pr = item.get("review_priority", "?")
            frozen[item["uco_node_id"]] = f"open pending_review amendment {aid} ({pr})"
        else:
            report["warnings"].append(f"freeze list: unrecognized entry {item!r}")
    return frozen
...
def main():
    ...
    ap.add_argument("--freeze-list", metavar="PENDING_JSON",
                    help="JSON export of open pending_review amendments "
                         "(GET /v1/compliance/amendments?status=pending_review) "
                         "or a bare array of UCO node ids. Frozen nodes are "
                         "EXCLUDED from the regenerated SQL seed — the gate "
                         "keeps enforcing the existing DB row until review "
                         "resolves (fail-closed, completes BLOCK semantics).")
    args = ap.parse_args()
    ...
    frozen = load_freeze_list(args.freeze_list, report) if args.freeze_list else {}
    report["frozen"] = []
    for n in valid:
        if n["uco_node_id"] in frozen:
            n["frozen"] = True
            report["frozen"].append(f"{n['uco_node_id']}: {frozen[n['uco_node_id']]}")
    unknown_frozen = set(frozen) - {n["uco_node_id"] for n in valid}
    for nid in sorted(unknown_frozen):
        report["warnings"].append(f"freeze list references unknown node {nid}")

    # JSON stays COMPLETE (canonical record, frozen flagged inline);
    # SQL EXCLUDES frozen nodes so their live DB rows are not overwritten
    # while review is pending.
    (out / "uco_nodes_seed.json").write_text(
        json.dumps(valid, indent=2), encoding="utf-8")
    regen = [n for n in valid if not n.get("frozen")]
    emit_sql(regen, out / "R__seed_uco_nodes.sql")
    ...
    rpt += [
        f"## Frozen nodes ({len(report['frozen'])}) — excluded from SQL regeneration", "",
        *([f"- {f}" for f in report["frozen"]] or ["- none"]), "",
    ]
    if report["monitors"]:
        rpt += ["## Firecrawl monitors", "", f"- {report['monitors']}", ""]
    (out / "seed_validation_report.md").write_text("\n".join(rpt), encoding="utf-8")

    print(f"valid={len(valid)} frozen={len(report['frozen'])} "
          f"sql_rows={len(regen)} rejected={len(report['rejected'])} "
