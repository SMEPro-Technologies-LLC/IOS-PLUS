...
| **Current Readiness Score** | **5** | `UcoResolver` is fully implemented: CIP→SOC→NAICS traversal, licensure lookup, crosswalk loading/validation, database queries. DB query layer now aligned with actual schema (uses `uco_crosswalk` table, V11 view column names, correct `uco_obligation_metadata.naics_code`). DB unit tests added. However, the `/v1/compliance/licensure/state-lookup` endpoint in `server.ts` returns a **stub** (hardcoded `unknown` status) and does not call `UcoResolver`. |
| **Blockers** | 1. API endpoint stubbed — `UcoResolver` is not imported or called by the middleware-engine. 2. `crosswalk.ts` loads from file paths; no cloud storage (GCS) adapter. 3. No caching layer for UDM queries (Redis unused). 4. No independent deployability. 5. V11 staging tables need seed data before real licensure lookups work. | |
...
| **Test Coverage** | **partial** | `scripts/db/verify-worm.js` exists. `docker-compose.test.yml` runs DB tests. WORM integration tests (`packages/cos-plus/src/worm-integration.test.ts`), invariant unit tests (`packages/cos-plus/src/invariant.test.ts`), and UCO resolver DB unit tests (`packages/uco-resolver/src/database.test.ts`) added in 2026-06-28 cleanup. |
| **SLOs** | **partial** | Terraform configures HA (`REGIONAL`), PITR, 7-day backups. No explicit SLO document. |
| **Backup/Restore Requirements** | **partial** | Terraform: `point_in_time_recovery_enabled=true`, `retained_backups=7`, transaction log retention 7 days. Target: RPO ≤ 5 min, RTO ≤ 4 hours. No restore drill results yet. |
| **Runbook Link** | `docs/PostgreSQL_Operations_Runbook.md` | Added 2026-06-28. Covers migration execution, rollback/roll-forward, restore checklist, WORM verification, remaining risks. |
| **Promotion Path** | Terraform apply per environment | Migrations run via `scripts/db/migrate.js` (custom, not Flyway). |
| **Production Wave** | Wave 1 | |
| **Current Readiness Score** | **7** | Schema is comprehensive. WORM triggers, indexes, UDM views, audit retention. Schema invariant checker now aligned with actual DDL (fixed stale column/trigger/index names). UCO resolver queries now use correct table (`uco_crosswalk`) and V11 column names. DB-focused automated tests added. Operational runbook added. Remaining gaps: custom migration runner unproven at scale, no Flyway/Liquibase, no migration rollback drill, no pgvector tuning, no connection pooling config, staging UDM tables need seed data. |
| **Blockers** | 1. Custom migration runner (`migrate.js`) — not proven at scale; no transaction safety documented. 2. No Flyway/Liquibase for production-grade migration control. 3. No migration rollback testing. 4. No pgvector index tuning (HNSW vs IVFFlat). 5. No connection pooling config (PgBouncer/Cloud SQL IAM). 6. V11 staging tables (`staging_cip_soc_state_license`, `staging_compact_participation`) need seed data loaded before UDM lookups return real results. | |
...
| PostgreSQL + pgvector | 7 | Custom migration runner, no Flyway; schema invariant checker and UCO resolver queries now aligned with actual schema; DB tests added |
